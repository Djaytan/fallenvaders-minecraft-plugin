    ...
    if (temp != null) {
      ...
      if (recipient != null) {
        MessageUtils.sendMessage(
            ...
            LangManager.getValue("string_receive_letter_notification", letterData.getAuthor()));
      }
      res = true;
    } else {
      MessageUtils.sendMessage(
          ...
          for (LetterData letter : sent) {
            ...
            if (recipient != null) {
              MessageUtils.sendMessage(
                  recipient,
                  MessageLevel.NOTIFICATION,
                  LangManager.getValue("string_receive_letter_notification", letter.getAuthor()));
            }
          }
          res = true;
  ...
  private static ItemStack getBookView(LetterData letterData) {
    ...
    letterHead.append(
        String.format(
            "%s%s:%s %s\n",
            ChatColor.BOLD,
            LangManager.getValue("string_author"),
            ChatColor.RESET,
    ...
    letterHead.append(
        String.format(
            "%s%s:%s %s\n",
            ChatColor.BOLD,
            LangManager.getValue("string_object"),
            ChatColor.RESET,
    ...
    if (temp != null) {
      ...
      if (recipient != null) {
        MessageUtils.sendMessage(
            recipient,
            MessageLevel.NOTIFICATION,
            LangManager.getValue("string_receive_item_notification", itemData.getAuthor()));
      }
