    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.RED_TERRACOTTA)
                .setName(String.format("%s%s" + CANCEL, ChatColor.WHITE, ChatColor.BOLD))
                .build(),
            consumer));
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.GREEN_TERRACOTTA)
                .setName(String.format("%s%s" + CONFIRM, ChatColor.RED, ChatColor.BOLD))
                .build(),
            onConfirmation(player, contents)));
  }

  public abstract Consumer<InventoryClickEvent> onConfirmation(
      Player player, InventoryContents contents);

  public abstract Consumer<InventoryClickEvent> onAnnulation(
      Player player, InventoryContents contents);
