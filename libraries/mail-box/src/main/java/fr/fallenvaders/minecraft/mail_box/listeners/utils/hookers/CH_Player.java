    ...
    this.setExecution(
        event -> {
          ...
          if (splitedMsg.size() == 1
              || splitedMsg.size() > 1 && ePlayer.hasPermission("mailbox.send.announce")) {
            ...
            if (wrongName == null) {
              MessageUtils.sendMessage(
                  ePlayer,
                  MessageLevel.INFO,
                  LangManager.getValue(
                      "information_chat_selection_recipients", identifiersList.getPreviewString()));
              parentInv.openInventory(ePlayer);
              this.stop();

            } else if (wrongName.equals(ePlayer.getName())) {
              MessageUtils.sendMessage(
                  ePlayer, MessageLevel.ERROR, LangManager.getValue("string_player_not_yourself"));

            } else {
              MessageUtils.sendMessage(
                  ePlayer,
                  MessageLevel.ERROR,
                  LangManager.getValue("string_player_not_found", wrongName));
            }

          } else {
            MessageUtils.sendMessage(
                ePlayer, MessageLevel.ERROR, LangManager.getValue("string_permission_needed"));
          }
        });
  }
}
