...
import fr.fallenvaders.minecraft.mail_box.inventory.inventories.PlayerSelectionInventory;
...
import fr.fallenvaders.minecraft.mail_box.listeners.utils.hookers.CH_Duration;
import fr.fallenvaders.minecraft.mail_box.listeners.utils.hookers.CH_LetterContent;
import fr.fallenvaders.minecraft.mail_box.listeners.utils.hookers.CH_LetterObject;
...
public class CreationInventory extends InventoryBuilder {
  ...
  private static final String CH_MESSAGE_START =
      LangManager.getValue("string_ch_letter_content_start");
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.PLAYER_HEAD)
                .setName(String.format("%s%s" + RECIPIENTS + ":", ChatColor.YELLOW, ChatColor.BOLD))
                ...
                if (player.hasPermission("mailbox.send.announce")) {
                  PlayerSelectionInventory selector =
                      new PlayerSelectionInventory(
                          this.getRecipients(), ChatColor.BOLD + TARGET_SELECTION + ":", this);
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.WHITE_BANNER)
                .setName(String.format("%s%s" + OBJECT + ":", ChatColor.YELLOW, ChatColor.BOLD))
            ...
            (e) -> {
              ...
              if (click == ClickType.LEFT || click == ClickType.RIGHT) {
                int msgSize = this.getObject().toString().length() - this.getObject().length() - 3;

                if (msgSize < 200) { // max length of objet
                  // set content
                  player.closeInventory();
                  CH_LetterObject ch =
                      new CH_LetterObject(CH_MESSAGE_START, this.getObject(), this);
                  ch.start(player);

                } else {
                  MessageUtils.sendMessage(
                      player, MessageLevel.ERROR, LangManager.getValue("string_msg_max_size"));
                }
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.WRITABLE_BOOK)
                .setName(String.format("%s%s" + MESSAGE + ":", ChatColor.YELLOW, ChatColor.BOLD))
                .addAutoFormatingLores(
                    this.getMessage().isEmpty() ? List.of(MESSAGE_EMPTY) : this.getMessage(), 35)
                .addAutoFormatingLore(LangManager.getValue("string_creation_message_help"), 35)
                .build(),
            e -> {
              ClickType click = e.getClick();
                ...
                if (msgSize < 700) {
                  if (click == ClickType.LEFT) {
                    ...
                    if (chCheck == null) {
                      player.closeInventory();
                      CH_LetterContent ch =
                          new CH_LetterContent(CH_MESSAGE_START, this.getMessage(), this, false);
                      ch.start(player);
                      ...
                      MessageUtils.sendMessage(
                          player,
                          MessageLevel.ERROR,
                          LangManager.getValue("string_end_last_entry_first"));
                    }

                  } else {
                    player.closeInventory();
                    CH_LetterContent ch =
                        new CH_LetterContent(CH_MESSAGE_START, this.getMessage(), this, true);
                    ch.start(player);
                  }
                } else {
                  MessageUtils.sendMessage(
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.BARRIER)
                .setName(String.format("%s%s" + RESET, ChatColor.RED, ChatColor.BOLD))
                .build(),
            e -> {
              ClickType click = e.getClick();
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.CLOCK)
                .setName(String.format("%s%s" + DURATION, ChatColor.YELLOW, ChatColor.BOLD))
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.FEATHER)
                .setName(String.format("%s%s" + SEND, ChatColor.YELLOW, ChatColor.BOLD))
                .build(),
            (e) -> {
              if (!this.getRecipients().isEmpty()) {
                if (!this.getMessage().isEmpty() || this.getItem() != null) {
                  ...
                  if (click == ClickType.LEFT) {
                    ...
                    SendConfirmationInventory sendInv =
                        new SendConfirmationInventory(
                            this,
                            this.getRecipients(),
