...
public class PlayerManager {
  ...
  public void load(Player player) {
    PlayerInfo playerInfo = playerInfoSql.tryRegister(player);
    this.addOnce(playerInfo);
  }

  public List<PlayerInfo> getOnlinePlayers(String toIgnore) {
    return this.getRegisteredPlayers(toIgnore).stream()
  ...
  public List<PlayerInfo> getRegisteredPlayers(String toIgnore) {
    ...
    if (toIgnore != null && !toIgnore.isEmpty()) {
      res.addAll(
          this.cache.stream()
              .filter(pi -> pi.getName() != null && !pi.getName().equals(toIgnore))
              .collect(Collectors.toList()));
