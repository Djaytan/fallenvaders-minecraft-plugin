...
public class PlayerSelectionInventory extends InventoryBuilder {
  ...
  private static final String HELP_DELETE_FILTER =
      LangManager.getValue("help_delete_player_filter");

  private IdentifiersList identifiersList;
  private boolean filterMode = true;

  public PlayerSelectionInventory(
      IdentifiersList identifiersList, String invTitle, InventoryBuilder parent) {
    super(ID, invTitle, 3);
  ...
  public void initializeInventory(Player player, InventoryContents contents) {
    if (this.getFilterMode() || player.hasPermission("mailbox.send.annouce")) {
      ...
      contents.set(
          ...
          6,
          ClickableItem.of(
              new ItemStackBuilder(CHOOSE_ALL_MATERIAL)
    ...
    contents.set(
        ...
        2,
        ClickableItem.of(
            new ItemStackBuilder(CHOOSE_PRECISE_PLAYER_MATERIAL)
                .setName(String.format("%s%s" + PRECISE, ChatColor.WHITE, ChatColor.BOLD))
                .build(),
            (e) -> {
    ...
    contents.set(
        ...
        ClickableItem.of(
            new ItemStackBuilder(Material.REDSTONE)
                .setName(
                    String.format(
                        "%s%s" + DISPLAYED_PLAYERS + ":", ChatColor.WHITE, ChatColor.BOLD))
