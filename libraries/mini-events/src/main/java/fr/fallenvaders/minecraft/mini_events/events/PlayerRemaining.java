...
public class PlayerRemaining {
    public static void PlayerLeft() {
        ...
        if (MiniEvents.getGame().getGameState() == GameState.STARTING || MiniEvents.getGame().getGameState() == GameState.PLAYING) {
            ...
            if (MiniEvents.getGame().getParticipants().size() >= 2) {
                for (UUID playerUUID : MiniEvents.getGame().getParticipants()) {
                    ...
                    players.sendMessage(eventPrefix + "§7Il reste désormais §b" + (MiniEvents.getGame().getParticipants().size()) + " joueurs !");
                }
            }
        }
    }
