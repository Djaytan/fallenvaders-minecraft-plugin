...
public class ArgSTART {
  ...
  public static void ArgStart(Player player, GameName eventName) {
    ...
    if (state == GameState.STARTING
      ...
      player.sendMessage(
          prefix + "§cUn événement est déjà en cours, impossible d'en lancer un autre.");
    }

    if (state == GameState.NOTSTARTED) {
      MiniEvents.getGame()
          .setGameState(GameState.WAITING); // On met l'évenement en WAITING (attente de joueurs).
      MiniEvents.getGame()
          .setGameName(
              eventName); // On dit que l'événement sera celui qu'on a écrit dans le commande.

      BroadcastEventAnnouncement.BroadcastMessage(
          MiniEvents.getGame()
              .getGameName()
              .getVisualName()
              .toUpperCase()); // Message d'annonce dans le chat.
    }
  }
}
