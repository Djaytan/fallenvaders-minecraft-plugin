...
public class ArgLEAVE {
    ...
    public static void ArgLeave(Player player) {
        ...
        if (state == GameState.NOTSTARTED) {
        ...
        } else if (state == GameState.WAITING && MiniEvents.getGame().getParticipants().contains(player.getUniqueId())) {
            ...
            Bukkit.broadcastMessage(player.getName() + "§7 a quitté l'évévenement §7(§c" + MiniEvents.getGame().getParticipants().size()
                + "§7/§c" + maxPlayers + "§7).");
        } else {
