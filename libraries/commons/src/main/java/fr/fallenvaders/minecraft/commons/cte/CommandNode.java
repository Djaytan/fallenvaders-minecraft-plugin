...
import fr.fallenvaders.minecraft.commons.cte.exceptions.CommandPermissionNotFoundException;
...
public class CommandNode {
   ...
   *
   * @param parent Le noeud parent, peut être null
   * @param argument L'argument à associer avec le noeud de la commande, non null
   * @param description La description de la commande, non null
   * @param executor L'exécuteur du noeud, peut être null
   * @param specificPermission La permission spécifique applicable à ce noeud uniquement, peut être
   *     null
   */
  CommandNode(
      CommandNode parent,
   ...
   *
   * @return True si le noeud fils correspond à un argument de saisie, false sinon
   */
  ...
  /**
   * Récupère et retourne la liste des arguments de la commande excluant l'argument racine.
   *
   * @return La liste des arguments de la commande excluant l'argument racine, non null
   */
   ...
   *
   * <ol>
   *   <li>On récupère la permission globale de la commande (ligne "permission" de la commande dans
   *       le fichier plugin.yml)
   *   <li>Pour chaque argument n'étant pas une saisie, on incrémente la permission par cet argument
   * </ol>
   *
   * <b><i>Exemple :</i></b><br>
   * Permission de la commande "test" pour le plugin Dornacraft-API dans le plugin.yml :
   * dornacraft.api.test<br>
   * Permission de la commande "test arg1 &lt;string&gt; arg2" : dornacraft.api.test.arg1.arg2
   *
  ...
  /**
   * Récupère et retourne la racine de l'arbre d'exécution associé au noeud courrant.
   *
   * @return La racine de l'arbre d'exécution associée au noeud courrant, non null
   */
   ...
   *
   * @param node Le noeud dont le père racine doit être trouvé, non null
  ...
  public String toString() {
    return "\n - "
        ...
        + getDescription()
        + "; Permission: "
        + (hasSpecificPermission() ? getSpecificPermission() : "none")
        + "; hasExecutor: "
        + (getExecutor() != null);
  }
   ...
   *
   * @param label Le label du noeud de la commande à rechercher, non null
   ...
   *
   * @param commandNode Le noeud à rechercher parmi les fils, non null
   ...
   *
   * @param commandNode Le noeud fils à vérifier, non null
  ...
  /**
   * Envoie à l'émetteur de la commande un message d'aide sur celle-ci à partir du noeud courant.
   *
   * @param sender L'émetteur de la commande, non null
   * @param label Le label de la commande, non null
   * @param page Le numéro de la page d'aide à afficher, non null
   */
  public void sendHelpMessage(CommandSender sender, String label, int page) {
    ...
    UtilsAPI.sendSystemMessage(
        MessageLevel.NORMAL,
        sender,
        UtilsAPI.COMMAND_HELP_HEADER,
        label + (nbPages > 1 ? " (" + page + "/" + nbPages + ")" : ""));

    // Envoie d'un message d'utilisation pour chaque commande de la page
    for (int i = NB_ELEM_PER_HELP_PAGE * (page - 1);
        i < NB_ELEM_PER_HELP_PAGE * page && i < getChildren().size();
        i++) {
      UtilsAPI.sendSystemMessage(
          MessageLevel.NORMAL,
          sender,
          "§a§l%s %s",
          UtilsAPI.CARAC_ARROW,
    ...
    if (nbPages > 1) {
      UtilsAPI.sendSystemMessage(
          MessageLevel.NORMAL, sender, UtilsAPI.COMMAND_HELP_CHANGE_PAGE, label);
    }
  }
  ...
  public void sendSpecificHelpMessage(CommandSender sender, String label) {
    UtilsAPI.sendSystemMessage(
        MessageLevel.NORMAL, sender, UtilsAPI.COMMAND_HELP_TRY_MESSAGE, getUtils(label, false));
  }

  /**
   * Construit et retourne le message d'utilisation de la commande accompagné d'une description ou
   * non.
   *
   * @param label Le label de la commande, non null
   * @param withDescription Indique si le message d'utilisation de la commande doit être accompagné
   *     d'une description ou non, non null
  ...
  public String getUtils(String label, boolean withDescription) {
    ...
    return sj
        + (!currentNode.getChildren().isEmpty() ? " §b..." : "")
        + ChatColor.GOLD
   ...
   *
   * @param node Le noeud correspondant à l'argument à traiter, non null
  ...
  public String getArgumentUsage(CommandNode node) {
    ...
    if (node.getArgument().isInputArg()) {
      Object[] args =
          node.getArgument().isRequired() ? new Object[] {"<", ">"} : new Object[] {"[", "]"};
   ...
   *
   * @param description La description à associer à la commande, non null
   */
   ...
   *
   * @param executor L'executor du noueud, peut être null
   */
  public void setExecutor(CommandNodeExecutor executor) {
   ...
   *
   * @param specificPermission La permission spécifique du noeud, peut être null
   */
