...
public class CommandTreeExecutor {
  ...
  public CommandTreeExecutor(String cmdLabel) throws CommandNotFoundException {
    ...
    setRoot(
        new CommandNode(
            null, new CommandArgument(cmdLabel), pluginCommand.getDescription(), null, null));
  }

  /**
   * Ajoute à l'arbre d'exécution la commande spécifiée en définissant le parant d'un argument sur
   * le noeud associé à l'argument précédent. Voici le formalisme à créer pour chaque ajout de
   * commande <b>à effectuer dans le constructeur de la classe Cmd</b> :
   *
   * <pre>
   * <code>
   ...
   *
   * <ul>
   *   <li><i><b>getCmdTreeExecutor() : </b>Appel de l'arbre d'exécution associé à l'instance
   *       héritante de DornacraftCommand.</i>
   *   <li><i><b>addSubCommand(...) : </b>Appel de la méthode permettant l'ajout d'une nouvelle
   *       commande à l'arbre d'exécution.</i>
   *   <li><i><b>new CommandNode(...) : </b>Créer un nouveau noeud dans l'arbre d'exécution. Chaque
   *       noeud correspond à un argument de la commande dans l'ordre spécifique.</i>
   *   <li><i><b>new CommandArgument(...) : </b>Définit le label principal de l'argument suivi
   *       éventuellement de ses aliases.</i>
   *   <li><i><b>new CommandArgument(...) avec CommandArgumentType : </b>Définit un argument de
   *       saisie en spécifiant le type d'argument attendu et s'il est obligatoire ou non</i>
   *   <li><i><b>new DornacraftCommandExecutor() : </b>Définit éventuellement l'exécuteur associé au
   *       noeud (obligatoire pour le dernier noeud de la commande sinon une exception sera
   *       levée)</i>
   *   <li><i><b>Permission spécifique à un noeud : </b>Il est possible de spécifier un argument
   *       spécifique à un noeud (attention : pas de propagation donc à le faire soit même si
   *       nécessaire). {@link CommandNode#CommandNode(CommandArgument, String, CommandNodeExecutor,
   *       String)}</i>
   * </ul>
   *
   * @param cmds La liste des noueuds à ajouter dans l'arbre d'exécution de la commande, non null
   */
  public void addSubCommand(CommandNode... cmds) throws InvalidCommandException {
    ...
    for (CommandNode subCmdNode : cmds) {
      ...
      // dans la liste
      CommandNode cmdNode =
          currentNode.getChildWithTypeArg(subCmdNode.getArgument().getType().getLabel());
      ...
      if (cmdNode == null) {
        cmdNode =
            new CommandNode(
                currentNode,
                new CommandArgument(
                    subCmdNode.getArgument().getType(), subCmdNode.getArgument().isRequired()),
                subCmdNode.getDescription(),
                subCmdNode.getExecutor(),
                subCmdNode.getSpecificPermission());
      }
    ...
    while (currentNode.getParent() != null) {
      if (currentNode.getArgument().isRequired()
          && !currentNode.getParent().getArgument().isRequired()) {
   ...
   *
   * @param args La liste des arguments de la commande saisis par l'émetteur, non null
  ...
  /**
   * Tente d'exécuter la commande en vérifiant la validité des arguments de saisie et l'importance
   * des arguments des noeuds fils du noeud à exécuter s'il ne s'agit pas d'une feuille de l'arbre
   * d'exécution. Traite les erreurs détectées lors des vérifications si il y en a, sinon lance
   * l'exécution de la commande.
   *
   * @param commandNode Le noeud correspondant à la commande à exécuter, non null
   * @param sender L'émetteur de la commande, non null
   * @param cmd La commande Bukkit correspondante, non null
   * @param label Le label de la commande à exécuter, non null
   * @param args La liste des arguments de la commande, non null
   * @throws Exception Si une erreur est détectée
   */
  public void executeCommand(
      CommandNode commandNode, CommandSender sender, Command cmd, String label, String[] args)
      throws Exception {
  ...
  /**
   * Vérifie la validité des arguments de la commande ainsi que la possession ou non des permissions
   * nécessaires par l'émetteur de la commande. Si toutes les conditions sont remplies, alors lance
   * la tentative d'exécution de la commande en faisant appel à la méthode {@link
   * CommandTreeExecutor#executeCommand(CommandNode, CommandSender, Command, String, String[])}.
   * Capte et traite toutes les erreurs non traitées.
   *
   * @param sender L'émetteur de la commande, non null
   * @param cmd Les informations générales de la commande, non null
   * @param label Le label de la commande, non null
   * @param args Les arguments de la commande émise, non null
   */
  public void checkCommand(CommandSender sender, Command cmd, String label, String[] args) {
    try {
      ...
      if (cmdNode != null) {
        ...
        try {
          if (!sender.hasPermission(cmdNode.getPermission(cmd))
              || !cmdNode.hasSpecificPermission()
              || sender.hasPermission(cmdNode.getSpecificPermission())) {
            try {
              executeCommand(cmdNode, sender, cmd, label, args);
            } catch (NumberFormatException e) {
              UtilsAPI.sendSystemMessage(
                  MessageLevel.ERROR, sender, UtilsAPI.NUMBER_FORMAT_INVALIDE);
            } catch (InvalidArgumentsCommandException e) {
              UtilsAPI.sendSystemMessage(
                  MessageLevel.ERROR, sender, UtilsAPI.COMMAND_INPUT_ARGUMENT_WRONG);
        ...
        UtilsAPI.sendSystemMessage(
            MessageLevel.NORMAL, sender, UtilsAPI.COMMAND_HELP_MESSAGE, label);
      }
   ...
   *
   * @param root Le noeud racine de l'arbre d'exécution, non null
   */
