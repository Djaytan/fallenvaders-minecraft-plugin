...
public class InventoryCR implements InventoryProvider {
  ...
  public void init(Player player, InventoryContents contents) {
    ...
    int nbrMutes = 0;
    int nbrTotal = 0;
    int nbrBansCanceled = 0;
    int nbrKicksCanceled = 0;
    int nbrMutesCanceled = 0;
        ...
        ClickableItem.of(
            ...
            e -> {
              if (e.isRightClick()) {
                ...
                for (String info : infos) whoClicked.sendMessage(" " + info);
                whoClicked.sendMessage("------------------------------------------");
              }
            }));
    ...
    if (selectedSanctions.size() > 0) {
      ...
      if (!p.isLast() && selectedSanctions.size() > 27) {
        contents.set(
            ...
            8,
            ClickableItem.of(
                GeneralUtils.pageSuivante(), e -> inventory.open(player, p.next().getPage())));
      }
    }
    ...
    if (sanction.getTSExpireDate() != null) {
      ...
      if (currentTime >= sanction.getTSExpireDate().getTime())
        if (sanction.getState().equals("cancel"))
          lores.add(
              ChatColor.WHITE
                  + "§l(Annulée) §f§mDate d'expiriation: §7§m"
                  + sdf.format(sanction.getTSExpireDate()));
        else
          lores.add(
              ChatColor.GREEN
                  + "(Terminée) Date d'expiriation: §7"
                  + sdf.format(sanction.getTSExpireDate()));
      else if (sanction.getState().equals("cancel"))
        lores.add(
            ChatColor.WHITE
                + "§l(Annulée) §f§mDate d'expiriation: §7§m"
                + sdf.format(sanction.getTSExpireDate()));
    ...
    if (!sanction.getState().equals("cancel")) {
      ...
      meta.setDisplayName(
          type.getVisualColor()
              + type.getVisualName()
              + ChatColor.GRAY
              + sdf.format((Date) sanction.getTSDate()));
    } else {
