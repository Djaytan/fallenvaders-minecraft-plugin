package fr.fallenvaders.minecraft.core.fight_session.commands;
      ...
      if (args[0].equalsIgnoreCase(ARG_RELOAD)) {
        if (sender.hasPermission("fightsession.reload")) {
          ...
          MessageUtils.sendMessage(
              sender,
              MessageLevel.ERROR,
              PluginController.getLangManager()
  ...
  private void sendHelpMsg(CommandSender sender) {
    if (sender.hasPermission("fightsession.help")) {
      ...
      MessageUtils.sendMessage(
          ...
          MessageLevel.ERROR,
          PluginController.getLangManager()
              .getValue("command_error_no_permission", "fightsession.help"));
    }
  }
  ...
  private void sendTimeLeft(FightSession playerCombat, Player target) {
    if (playerCombat != null) {
      MessageUtils.sendMessage(
          ...
          MessageLevel.INFO,
          PluginController.getLangManager()
              .getValue("command_time_1", playerCombat.getFormattedTime()));
    } else {
      MessageUtils.sendMessage(
          target,
          MessageLevel.INFO,
